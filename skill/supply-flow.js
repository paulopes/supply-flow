...
function arrH(x1, y1, cx, cy, x2, y2, sz) {
  sz = sz || 7; var tx = x2 - cx, ty = y2 - cy, l = Math.sqrt(tx * tx + ty * ty) || 1;
...
function placeLabels(N, nP, vb) {
  ...
  N.forEach(function (node) {
    var pos = nP[node.id]; if (!pos) return;
    ...
    cands.forEach(function (c) {
      ...
      placed.forEach(function (pr) {
        var ox = Math.max(0, Math.min(r.x + r.w, pr.x + pr.w) - Math.max(r.x, pr.x));
        var oy = Math.max(0, Math.min(r.y + r.h, pr.y + pr.h) - Math.max(r.y, pr.y)); sc += ox * oy * 2;
      });
      N.forEach(function (o) {
        if (o.id === node.id) return; var op = nP[o.id]; if (!op) return; var cr = NR + 5;
...
function buildPop(node, rc, x, y, vb, nodeId) {
  ...
  if (r) {
    i += '<div style="margin-top:3px;padding:6px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:5px;font-size:9.5px;line-height:1.45;">';
    ...
    if (r.cslSearchName) {
      var u = 'https://www.trade.gov/data-visualization/csl-search#/search?q=' + encodeURIComponent(r.cslSearchName);
  ...
  if (node.coveredList) {
    i += '<div style="margin-top:5px;padding:5px 6px;background:#fef2f2;border:1px dashed #dc2626;border-radius:5px;"><div style="font-weight:700;font-size:10px;color:#dc2626;margin-bottom:1px;">\u26A0 FCC Covered List Entity</div>';
...
function buildSVG(cfg) {
  ...
  if (cfg.product) {
    var pt = cfg.product.name + (cfg.product.specs ? ' \u2014 ' + cfg.product.specs : '');
  ...
  if (!(cfg.map && cfg.map.showGrid === false)) {
    for (var lon = -270; lon <= 90; lon += gi) {
      var gx = lonToX(lon > 90 ? lon : lon);
      if (lon >= -180 && lon <= 180) { gx = (lon + 270) / 360 * MW; } else continue;
      s += '<line x1="' + gx.toFixed(1) + '" y1="0" x2="' + gx.toFixed(1) + '" y2="' + MH + '" class="sf-grid"/>';
    }
    for (var lat = -90; lat <= 90; lat += gi) {
      if (lat === 0) continue; var gy = latToY(lat);
  ...
  F.forEach(function (f) {
    var fp = nP[f.from], tp = nP[f.to]; if (!fp || !tp) return;
  ...
  N.slice().sort(function (a, b) { return (a.coveredList ? 1 : 0) - (b.coveredList ? 1 : 0); }).forEach(function (node) {
    ...
    var pl = plM[node.id]; if (pl) {
      if (pl.cb) s += '<text class="sf-covered-badge" x="' + pl.cb.x.toFixed(1) + '" y="' + pl.cb.y.toFixed(1) + '" text-anchor="' + pl.cb.a + '">\u26A0 Covered</text>';
...
function hierCss() {
  return '\n' +
    ...
    '.sf-hier-entity{flex:0 0 auto}\n' +
    '.sf-hier-card{background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:10px 14px;min-width:180px;max-width:260px}\n' +
